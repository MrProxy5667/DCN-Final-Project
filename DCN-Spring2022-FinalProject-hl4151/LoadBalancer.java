/**
* LoadBalacer.java
* This file can configure flow tables in newly added switches to handle ARP and TCP
* packets that uses virtual IP as destination addresses.
* For forwarded ARP requests and TCP packets, actions will be taken according to the
* packet type;
* For ARP, an ARP reply will be directly sent back to the requesting client;
* For TCP SYN, a rule to transform between real and virtual IP & MAC addresses will
* be added into the switch;
* For non-SYN TCP packets, a TCP RESET packet will be directly sent back to the client;
* Other packets will be ignored.
* Details can be seen in lines 154 to 355.
* 
* Created & edited by Haochen Li (mailto: hl4151@nyu.edu)
* 05/16/2022
*/
		...
		IOFMessageListener
{
	...
	public void switchAdded(long switchId) 
	{
		...
		/*********************************************************************/
		// Installing rules for sending packets to virtual load balancer IPs;
		// Matching condition: destination IP is this virtual IP;
			...
			IOFSwitch sw, OFMessage msg, FloodlightContext cntx) 
	{
		...
		// Send ARP replies when the packet type is ARP;
		...
		// Install rewrite rule for a selected host;
		else if(curPacketType == OFMatch.ETH_TYPE_IPV4) {
			// Get the payload of current ethPkt as an IPv4 packet, and obtain its destination IP (virtual IP value);
			IPv4 ipContent = (IPv4) ethPkt.getPayload();
			int destVirtualIP = ipContent.getDestinationAddress();
			if(ipContent.getProtocol() != IPv4.PROTOCOL_TCP || !(instances.keySet().contains(destVirtualIP))) {
				return Command.CONTINUE;
			}

			// Get the payload of this IPv4 packet, which is a TCP packet;
			TCP tcpContent = (TCP) ipContent.getPayload();
			// If this packet is SYN, then install corresponding rewriting rules;
